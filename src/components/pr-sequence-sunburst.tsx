...
/* ------------------------------------------------------------------ */
/*  Arc fill color by add/del ratio                                    */
/* ------------------------------------------------------------------ */

function arcFill(d: HierarchyRectangularNode<SeqNode>): string {
  const data = d.data;
...
export function PrSequenceSunburst({
  pr,
}: {
  pr: PullRequest | null;
}) {
  ...
  const partitionRoot = useMemo(() => {
    ...
    ) as HierarchyRectangularNode<SeqNode>;
  }, [hierarchy]);
  ...
  useEffect(() => {
    ...
    const arc = d3
      .arc<HierarchyRectangularNode<SeqNode>>()
      .startAngle((d) => d.x0)
      .endAngle((d) => d.x1)
      .padAngle(2 / radius)
      .innerRadius((d) => Math.sqrt(d.y0))
      .outerRadius((d) => Math.min(Math.sqrt(d.y1), radius) - 1);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <svg
            ...
            width={560}
            height={560}
            aria-label="PR file sequence sunburst"
          />
          {(focus ?? hovered) && (
